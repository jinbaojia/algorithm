 ...
 * 时间复杂度 O(nlogn)
 * 空间复杂度O(1)
 * leetCode 147 https://leetcode-cn.com/problems/sort-list/
 * */

const {linkByArray, arrayByLink, ListNode} = require("./Link");
const mergeTwoOrderedLink = require("./合并有序链表/orderedLinkedList");
const list =
...
const sortList = function (head) {
    ...
    const midNext = slow.next;
    slow.next = null;
    return mergeTwoOrderedLink(sortList(head), sortList(midNext))
};
// console.log(arrayByLink(sortList(head)));
 ...
 * 每次将链表拆分成若干个长度为subLength 的子链表（最后一个子链表的长度可以小于subLength），
 * 按照每两个子链表一组进行合并，合并后即可得到若干个长度为2subLength×2 的有序子链表（最后一个子链表的长度可以小于subLength）。
...
const sortList2 = function (head) {
    ...
    for (let subLength = 1; subLength < length; subLength <<= 1) {
        ...
        while (cur) {
            ...
            pre.next = mergeTwoOrderedLink(head1, head2);
            while (pre.next) {
                pre = pre.next;
            }
        }
...
console.log(arrayByLink(sortList2(head)));
